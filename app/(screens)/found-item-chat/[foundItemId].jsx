...
const FoundItemChatPage = () => {
  const params = useLocalSearchParams();
  const { foundItemId } = params;
  ...
  useEffect(() => {
    ...
    const fetchData = async (user) => {
      ...
      try {
        const response = await fetch(`${BASE_URL}/api/found-items-chat/chat`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        const data = await response.json();
        setChatId(data.foundItemChat._id);
...
export default FoundItemChatPage;
